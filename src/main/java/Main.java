...
public class Main {
        ...
        for (int i = 0; i < sz / portion_size; i++) { // цикл по подвыборкам, их количество = размер массива котировок / portion_size
            ...
            for (int k = 0; k < 4; k++) { // цикл по стратегиям
                sampling[i].profit[k] = budget * (
                        strategy[k][0] * ((1 / rub_usd[l].open) * rub_usd[l + portion_size - 1].close - 1) +
                                strategy[k][1] * ((1 / huf_usd[l].open) * huf_usd[l + portion_size - 1].close - 1) +
                                strategy[k][2] * ((1 / nok_usd[l].open) * nok_usd[l + portion_size - 1].close - 1) +
                                strategy[k][3] * ((1 / sgd_usd[l].open) * sgd_usd[l + portion_size - 1].close - 1)
                );
            }
            ...
            for (int k = 0; k < 4; k++) { // цикл по стратегиям
                ...
                for (int j = l; j < l + portion_size; j++) {
                    x_ = x_ + strategy[k][0] * rub_usd[j].close + strategy[k][1] * huf_usd[j].close +
                            strategy[k][2] * nok_usd[j].close + strategy[k][3] * sgd_usd[j].close;
                }
                x_ = x_ / portion_size;
                ...
                double d = 0;
                for (int j = l; j < l + portion_size; j++) {
                    d = d + (strategy[k][0] * rub_usd[j].close + strategy[k][1] * huf_usd[j].close +
                            strategy[k][2] * nok_usd[j].close + strategy[k][3] * sgd_usd[j].close - x_) *
                            (strategy[k][0] * rub_usd[j].close + strategy[k][1] * huf_usd[j].close +
                                    strategy[k][2] * nok_usd[j].close + strategy[k][3] * sgd_usd[j].close - x_);
                }
    ...
    public static void InitData() {
             ...
             */
        try {
            List<String> pairRUB_USD = Files.readAllLines(Paths.get(RESOURSES_PATH + "RUBUSD_220825_221225.csv"));
            List<String> pairHUF_USD = Files.readAllLines(Paths.get(RESOURSES_PATH + "HUFUSD_220825_221225.csv"));
            List<String> pairNOK_USD = Files.readAllLines(Paths.get(RESOURSES_PATH + "NOKUSD_220825_221225.csv"));
            List<String> pairSGD_USD = Files.readAllLines(Paths.get(RESOURSES_PATH + "SGDUSD_220825_221225.csv"));

            // Первая строка - заголовок
            sz = min(pairRUB_USD.size(), pairHUF_USD.size(), pairNOK_USD.size(), pairSGD_USD.size()) - 1;

            rub_usd = new CurrencyData[sz];
            huf_usd = new CurrencyData[sz];
            nok_usd = new CurrencyData[sz];
            sgd_usd = new CurrencyData[sz];

            for (int i = 1; i < sz; i++) {
                String[] p_rub_usd = pairRUB_USD.get(i).split(",");
                String[] p_huf_usd = pairHUF_USD.get(i).split(",");
                String[] p_nok_usd = pairNOK_USD.get(i).split(",");
                String[] p_sgd_usd = pairSGD_USD.get(i).split(",");
                rub_usd[i - 1] = new CurrencyData(parseDate(p_rub_usd[2]), Double.parseDouble(p_rub_usd[4]), Double.parseDouble(p_rub_usd[7]));
                huf_usd[i - 1] = new CurrencyData(parseDate(p_huf_usd[2]), Double.parseDouble(p_huf_usd[4]), Double.parseDouble(p_huf_usd[7]));
                nok_usd[i - 1] = new CurrencyData(parseDate(p_nok_usd[2]), Double.parseDouble(p_nok_usd[4]), Double.parseDouble(p_nok_usd[7]));
                sgd_usd[i - 1] = new CurrencyData(parseDate(p_sgd_usd[2]), Double.parseDouble(p_sgd_usd[4]), Double.parseDouble(p_sgd_usd[7]));
            }
